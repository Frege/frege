...
It provides conversion from and to \texttt{Int} values, successor and predecessor functions and the operations $enumFrom$, $enumFromTo$, $enumFromThen$ and $enumFromThenTo$ to construct \hyperref[aseq]{arithmetic sequences}. \texttt{Enum} is a subclass of \texttt{Ord}.
...
\hspace{0.5cm}enumFromTo $a$ $b$ = \textbf{if} $a \le{} b$ \textbf{then} $a$:enumFrom $($succ $a)$ $b$ \textbf{else} \bracka{}\brackz{}\\
