...
\end{enumerate}
In all other repects, native functions are indistinguishable from all other functions with the same type.
...
\end{flushleft}
\par where neither $t_o$ nor $t_r$ are function types, $t_o$ is associated with a java reference type and $j$ is an identifier that is valid in \java{}. Then the application $v$ $x$ will be compiled to a field access expression on the \java{} object that is denoted by expresion $x$.
}
