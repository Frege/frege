...
\subsection{Mutable Data and the ST Monad}

Consider the problem of concatenating a list of strings. It could be solved in a purely functional way in \frege{}  thus:
...
However, this solution has a drawback. The operator (++) that concatenates 2 strings, uses \java{}s + operator. 
This opearator is notorious for being quite inefficient, 
since it will create a new, appropriately sized string object and then copy the character data of both operands.

The standard practice in \java{} for problems like the above is therefore to create an intermediate \texttt{StringBuilder} object, and to append the individual strings in a loop. Once all strings are appended, the resulting string can be retrieved from the \texttt{StringBuilder}, which can subsequently be garbage collected.

Despite this approach does mutate mutable data (i.e, the \texttt{StringBuilder}), the overall effect is pure.
The \emph{ST monad} is designed to make it possible to construct such overall pure computations that employ impure means, as long as the side effects are confined to mutation of \emph{temporary} mutable data.
...
Depending on the type and the form of the java item, different \java{} constructs are supported.

\subsection{Static Field Access Expressions}
