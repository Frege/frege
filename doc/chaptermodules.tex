...
\index{function!main}
The generated \java{} code for a \frege{} package contains static fields, methods and inner classes that correspond to \frege{} values, functions and types defined in that package. If the source code contained a top level binding for the name \emph{main}, then the compiler creates a method with the signature \exq{
    static void main(java.lang.String[] args) 
}
which converts the argument array to a list, evaluates the \emph{main} function from the \frege{} code and runs the resulting IO action. The only allowed type for \emph{main} is \exq{
    main :: [String] -> IO ()
}
...
\hasdiff{The \emph{main} value is a function that takes a list of strings and returns an IO () action. There is no library function to access the command line arguments. The \emph{main} function can appear in any package. In fact, a program can consist of multiple packages each defining its own \emph{main} function. To run a specific \emph{main} one passes the package name to an appropriate JVM launcher.} 
