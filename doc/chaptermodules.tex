...
import Imported (Notes)   -- data type Notes and its non-private constructors\\
import Imported (Notes.C) -- just the data constructor
}

\paragraph*{Controlling import of data constructors and class members}

A class name in an import list can be followed by a list of class operations that should be available in the namespace of the importing package. If the list is empty, only the class name itself is imported, access to the class operations are possible through names that are qualified with the class name. If the list is missing, all class operations are available.

Likewise, a type name in an import list can be followed by a list of constructor names that should be available in the namespace of the importing package. If the list is empty, no constructor names are made available. If the list is missing, all constructor names are available.
...
Like with items in the import list, class operations can be renamed or reexported (see below for explanation).  An alternative solution for the constructor name clash problem would be:
...
\paragraph*{Empty import list} If the import list is empty, no named items are imported to the current namespace. Nevertheless, as before, all items of the imported package can be accessed with qualified names.
...
This is useful to avoid name clashes or to adapt naming conventions. However, error messages may report the original qualified name.

Multiple aliases are possible for a single item.

\example{import math.Rational except(gcd = greatestCommonDivisor, gcd = ggT)}
...
\subsection{Re-exporting imported items}
...
If the import list is preceded by the word "\term{except}", all public items but the named ones are made available for unqualified access in the current package.
...
It is advised to use the import feature described above to avoid such clashes.
