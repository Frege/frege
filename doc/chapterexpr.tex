% $Revision: 132 $
% $Id: chapterexpr.tex 132 2011-01-30 15:43:37Z ingo $
...
\rul{term} \nont{qvarid} \gcom{variable}
  ...
  \alt{} \nont{record} \gcom{see \autoref{recordterm}}
  %\alt{} \sym{(} \nont{expr} \sym{,} \liste{\nont{expr}}{\sym{,}} \sym{)}
\end{flushleft}
...
An expression of arbitrary complexity becomes a term syntactically when enclosed in parentheses. It is also possible to explicitly type an expression, this is explained later in \autoref{types}.
...
\rul{list} \sym{[]}  \gcom{constructor for the empty list}
  \alt{} \bracka{}\liste{\nont{expr}}{\sym{,}}\brackz{}
  \alt{} \sym{[} \nont{expr} \sym{$|$} \liste{\nont{lc-qual}}{\sym{,}} \sym{]} \gcom{list comprehension}\\
\rul{lc-qual} \nont{pat} \sym{<-} \nont{expr} \gcom{generator}
  \alt{} \nont{expr}    \gcom{guard}
\end{flushleft}
...
\rul{primary} \nont{term}
  ...
  \alt \nont{tycon} \sym{.} \sym{\{}\nont{varid} \sym{=} \nont{expr}\sym{\}} \gcom{update value function}
...
The syntax reveals that the \sym{.} works like a left associative operator. This is so that primary expressions can be chained, much like in conventional programming languages. For example \exq{ a.[42].age.negate} is the negated value associated with the \texttt{age} field of the value in the 43rd element of the array \texttt{a}.

\hasdiff{The \sym{.} is not a binary operator, but a hard wired syntactic element used to form primary expressions. The function composition operator is \sym{$\bullet$}}
...
The expression $T$.\{$field$\sym{?}\} denotes the function that checks, whether a value $v$ with a type $T u_1 \cdots u_k$  where $k>=0$ was constructed with a \hyperref[fieldconstructor]{data constructor} in whose field list appears the label $field$.
...
In both cases, $v$ must have been constructed with a \hyperref[fieldconstructor]{data constructor} in whose field list appears the label $field$. Otherwise, the result is undefined, and an attempt to evaluate it will cause a pattern match failure.
...
\begin{figure}[hbt]
\trans{
Let it be known that $v$ is of type $t$, and let $t$ be an algebraic data type with constructors $C_1$, $C_2$ and $C_3$. Let label $f$ appear as the first of two fields in $C_1$ and as the second of three fields in $C_2$\footnotemark{}. Then the following transformation is applied:
\begin{tabular}{lcp{0.3\textwidth}}
$T$.\{$f$=\} & $=$ & $\backslash v\backslash u \rightarrow$ \textbf{case} $v$ \textbf{of \{} \\
& & \hspace{1cm} $C_1$ $a_1$ $a_2 \rightarrow C_1$ $u$ $a_2$;\\
& & \hspace{1cm} $C_2$ $a_1$ $a_2$ $a_3 \rightarrow C_2$ $a_1$ $u$ $a_3$ \textbf{\}}\\
$T$.\{$f$$\leftarrow$\} & $=$ & $\backslash v\backslash g \rightarrow$ \textbf{case} $v$ \textbf{of \{} \\
& & \hspace{1cm} $C_1$ $a_1$ $a_2 \rightarrow C_1$ ($g$ $a_1$) $a_2$;\\
& & \hspace{1cm} $C_2$ $a_1$ $a_2$ $a_3 \rightarrow C_2$ $a_1$ ($g$ $a_2$) $a_3$ \textbf{\}}\\
$v$.\{$f$=\} & $=$ & ($T$.\{$f$=\} $v$)\\
...
$v$.\{$f_1=u_1$, & $=$ & $v$.\{$f_3=u_3$\}\\
\hspace{0.5cm}$f_2\leftarrow u_2$,  &  & \hspace{0.5cm}.\{$f_1=u_1$\}\\
\hspace{0.5cm}$f_3=u_3$, $\cdots$\} & & \hspace{0.5cm}.\{$f_2\leftarrow u_2$\}.$\cdots$\\
\end{tabular}    
...
\rul{binex$_i$}
   ...
   \alt \nont{binex$_{i-1}$} \nont{opnone$_i$}  \nont{binex$_{i-1}$}
   \alt \nont{binex$_{i-1}$}\\
\rul{opleft$_i$} \nont{qlexop} \gcom{left associative with precedence $i$}\\
\rul{opright$_i$} \nont{qlexop} \gcom{right associative with precedence $i$}\\
\rul{opnone$_i$} \nont{qlexop} \gcom{non-associative with precedence $i$}\\
\end{flushleft}

The syntax shows that function application has higher precedence than any binary expression, followed by binary expressions of precedence 1 down to 16.
...
\begin{code}
lst = [("John", 42), ("Mary", 36), ("Bill", 16)]
// sort the list by names
sorted = sortBy (\(nm1,_)\(nm2,_) -> nm1 <= nm2) lst
\end{code}
...
\section{Imperative Expressions} \index{expression!do}
...
\hasdiff{
\frege{} cannot guarantee that expressions do not have side effects, since arbitrary \java{} classes and methods may be used. There is currently no way to distinguish between "pure" and "impure" \java{} methods.
}

\begin{flushleft}
\rul{impex} \term{do} \bracea{} \liste{\nont{dqual}}{\sym{;}} \bracez{} \term{for} \nont{expr}\\
...
\trans{\\
\begin{tabular}{llcl}
1 & \texttt{do \{} $d_1$; $d_2$; $\cdots$ \texttt{\} for} $e$ & $=$ &
\texttt{do \{} $d_1$ \texttt{\} for}\\
&&& \texttt{ do \{} $d_2$; $\cdots$ \texttt{\} for} $e$\\
2a & \texttt{do \{} $e_1$ \texttt{\} for} $e_2$ & $=$ &
\texttt{case} $e_1$ \texttt{ of \{}  !$u$ \texttt{$\rightarrow$} $e_2$ \texttt{\}}\\
2b & \texttt{do \{} $p_1$ $\leftarrow$ $e_1$ \texttt{\} for} $e_2$ & $=$ &
\texttt{case} $e_1$ \texttt{ of \{}  !$u$@$p_1$ \texttt{$\rightarrow$} $e_2$ \texttt{\}}\\
\end{tabular}
\par where the $d_i$ range over qualifiers, $e_i$ over expressions, $p_i$ over patterns and $u$ stands for a fresh variable that is different from all other variables used in the program.
}

Rule 1 splits an imperative expression with multiple qualifiers into a series of imperative expressions that have exactly one qualifier each.
...
\rul{pterm} \nont{conid}    \gcom{constructor}
    ...
    \alt \sym{(} \nont{pattern} \opt{\sym{::} \nont{type}} \sym{)}
    \alt \sym{(} \sym{)}    \gcom{unit constructor}
    \alt \nont{tupel}       \gcom{see \autoref{tuplepattern}}
    \alt \nont{list}        \gcom{see \autoref{listpattern}}
    \alt \nont{record}      \gcom{see \autoref{recordpattern}}\\

 \rul{pvar} \nont{varid}
    \alt \regex{\_}         \gcom{anonymous variable}
    \alt \nont{pvar} \sym{.} \sym{\{} \nont{varid} \opt{\sym{=} \nont{pattern}} \sym{\}}
    \alt \sym{!} \nont{pvar}    \gcom{strict pattern}\\
...
\exq{case e of [x,\_,\_] -> x==0  }
is equivalent to:
\exq{case e of [x,y,z] -> x==0  }
