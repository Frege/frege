...
\end{tabular}
\par where the $d_i$ range over local declarations, $q_1; $\cdots$; $q_i$ is a non empty list of arbitrary qualifiers, $e$ stands for an expressions and $p$ for a pattern.
}
...
%Note that there is an ambiguity in the grammar. In the expression $\backslash p$ \arrow{} $e$ :: $t$ the parserhas two choices when it sees the double colon:
%\begin{itemize}
%\item the parser could reduce the lambda and the whole expression as ($\backslash p$ \arrow{} $e$) :: $t$ which would take $t$ to be the type of the lambda.
%\item or it could shift the double colon, and parse the $\backslash p$ \arrow{} ($e$ :: $t$) which would take $t$ to be the type of $e$.
%\end{itemize}

%This ambiguity is resolved in favor of the latter choice.
...
In an ordinary constructor application, the arity of the constructor must match the number of sub-patterns associated with it; one cannot match against a partially-applied constructor.
...
\end{code}
\item \label{strict pattern} A strict pattern \sym{!}$p$ is matched against a value $v$ by first evaluating $v$ and then matching $p$ against the result. Because evaluation of a value can diverge, matching a strict pattern can diverge in cases  when matching a normal pattern would not.
...
\begin{itemize}
\item A strict pattern is never irrefutable, because the match can diverge.
\item Variable and wildcard patterns are irrefutable.
\item $p$::$t$ is irrefutable, if $p$ is
\item $v$@$p$ is irrefutable if $p$ is
\item $C$ $p_0$ $\cdots$ $p_k$ is irrefutable if all subpatterns are irrefutable and if $C$ is the only constructor of the algebraic data type, i.e. if the type is a so called product type, of which tuple types are the most prominent representatives.
\item All other patterns are refutable.
\end{itemize}

Irrefutable patterns are said be matched lazily. For example in
\begin{code}
    swap (a,b)   = (b,a)
    x = swap undefined
\end{code}
the value of $x$ is the tuple $(fst$ $undefined$, $snd$ $undefined)$, that is, binding of the local variables in $swap$ does not actually extract the values from the argument, but merely notes how they could be extracted should the need arise.
