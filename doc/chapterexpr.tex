...
\hasdiff{The \sym{.} is not a binary operator, but a hard wired syntactic element used to form primary expressions. The function composition operator is \sym{$\bullet$} or \sym{<\symbol{126}}}
...
Irrefutable patterns are matched lazily. For example in
\begin{code}
    swap (a,b)   = (b,a)
    x = swap undefined
\end{code}
